...
2. 松手后直接后台导入并复制 PDF、写入 catalog；无 TeX 时会尝试生成 `PAPER.md`。

PDF 拖到窗口任意区域都会导入当前 Papers 目标目录；Library 表和文件树中的
`papers/` 组织夹会优先使用它们各自的目标目录。识别元数据、重命名和版面分析在
后台任务中继续。图片等非 PDF 拖到 Library 不会出现 overlay。
